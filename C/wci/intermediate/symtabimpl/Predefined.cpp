...
void Predefined::initialize_types(SymTabStack *symtab_stack)
{
    ...
    // Type double. (basically implemented as a 'real')
    double_id = symtab_stack->enter_local("double");
    double_type = TypeFactory::create_type((TypeForm) TF_SCALAR);
    double_type->set_identifier(double_id);
    double_id->set_definition((Definition) DF_TYPE);
    double_id->set_typespec(double_type);
